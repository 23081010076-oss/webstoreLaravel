<div x-data="{ animate: false }" @cart-Update.window="animate = true; setTimeout(() => animate = false, 500)">
                        <!-- Button Group -->
                    <div
                        class="relative flex flex-wrap items-center gap-x-1.5 md:ps-2.5 mt-1 md:mt-0 md:ms-1.5 before:block before:absolute before:top-1/2 before:-start-px before:w-px before:h-4 before:bg-gray-300 before:-translate-y-1/2 dark:before:bg-neutral-700">
                        <a :class="animate ? 'animate-cart-bounce bg-green-600 text-white' : 'bg-blue-600 text-white dark:bg-blue-500'"
                            class="inline-flex items-center gap-x-1.5 py-1.5 px-3 rounded-full text-xs font-medium transition-all duration-300 transform"
                            href="{{ route('cart') }}" wire:navigate>
                            <svg xmlns="http://www.w3.org/2000/svg" class="shrink-0 size-4" width="24"
                                height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                                stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-shopping-cart">
                                <circle cx="8" cy="21" r="1" />
                                <circle cx="19" cy="21" r="1" />
                                <path
                                    d="M2.05 2.05h2l2.66 12.42a2 2 0 0 0 2 1.58h9.78a2 2 0 0 0 1.95-1.57l1.65-7.43H5.12" />
                            </svg>
                            {{ $count }} Item
                        </a>
                    </div>
                    <!-- End Button Group -->
</div>
